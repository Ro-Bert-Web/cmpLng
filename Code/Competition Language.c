...
//Corrects the floor function at the negative numbers
#define flr(cmpLngX) (cmpLngX >= 0) ? floor(cmpLngX) : ceil(cmpLngX)

//Corrects the ceil function at the negative numbers
#define cel(cmpLngX) (cmpLngX >= 0) ? ceil(cmpLngX) : floor(cmpLngX)

//A custom modulous function that continues the pattern onto negative numbers
#define mod(cmpLngX, cmpLngY) cmpLngX - flr(cmpLngX / cmpLngY) * cmpLngY
...
time1[T4] = 0;\
wait1Msec(cmpLngSpeedAccuracy);\
cmpLngVar = nMotorEncoder[cmpLngNcdr] * 1000;\
cmpLngVar /= time1[T4] * 6

//Sets x to the speed of y, can be up to twice as accurate if using high strength 393
#define rawEncoderSpeed(cmpLngVar, cmpLngNcdr)\
nMotorEncoder[cmpLngNcdr] = 0;\
time1[T4] = 0;\
wait1Msec(cmpLngSpeedAccuracy);\
cmpLngVar = nMotorEncoderRaw[cmpLngNcdr] * 60000;\
cmpLngVar /= time1[T4];\
cmpLngVar /= (motorType[cmpLngNcdr]==tmotorVex393_HBridge || motorType[cmpLngNcdr]==tmotorVex393_MC29) ? 627.2 : ((motorType[cmpLngNcdr]==tmotorVex393HighSpeed_HBridge || motorType[cmpLngNcdr]==tmotorVex393_MC29) ? 392 : 240.448)
